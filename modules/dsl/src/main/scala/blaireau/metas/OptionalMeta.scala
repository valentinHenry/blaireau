...
import shapeless.{HList, HNil, LabelledGeneric, Lazy, Poly1}
import skunk.Codec
...
object OptionalMeta {
  ...
  def apply[T0, MF0 <: HList, EF0 <: HList, IF0 <: HList, IMF0 <: HList, IEF0 <: HList](
    ...
    override def internal: Meta.Aux[T0, IF0, IMF0, IEF0] = _internal
  ...
  implicit final def genericMetaEncoder[
    ...
    lInternalMeta: Lazy[Meta.Aux[A, IF, IEF, IMF]],
    tw: OptionalTwiddler.Aux[A, CT]
  ): OptionalMeta.Aux[A, MF, EF, IF, IEF, IMF] = {
    val internal: Meta.Aux[A, IF, IEF, IMF]      = lInternalMeta.value
